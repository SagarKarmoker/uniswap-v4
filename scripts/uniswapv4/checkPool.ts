...
async function checkPool(provider: any, poolKey: any, tokenSymbol: string, decimals: number) {
  ...
  try {
    ...
    const result = await (quoter as any).quoteExactInputSingle.staticCall({
      poolKey,
      zeroForOne: true,
      exactAmount: ethers.parseEther("1"),
      hookData: "0x"
    });
    
    console.log(`✅ ${tokenSymbol} pool EXISTS!`);
    console.log(`   Quote: 1 ETH → ${ethers.formatUnits(result[0], decimals)} ${tokenSymbol}`);
...
async function main() {
  ...
  for (const pool of poolsToTest) {
    const exists = await checkPool(provider, pool.poolKey, pool.name, pool.decimals);
    if (exists) {
